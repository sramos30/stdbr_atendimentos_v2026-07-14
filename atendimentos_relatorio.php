...
<?php 
	...
	if( isset($_GET["visualizar"]) || isset($_GET["geraexcel"]) )
		get_GET_array( array( 'data', 'data2', 'produtos_id', 'terminais_id', 'difmenor', 'difmaior', 'falta', 'excesso', 'visualizar', 'geraexcel', 'msg' ) );
	else
		get_POST_array( array( 'data', 'data2', 'produtos_id', 'terminais_id', 'difmenor', 'difmaior', 'falta', 'excesso', 'visualizar', 'geraexcel', 'msg' ) );
	...
	if( isset($geraexcel) ) {
		...
		$url = $_SERVER["REQUEST_SCHEME"]."://".$_SERVER["HTTP_HOST"]."/atendimentos/api/atendimentos.php?mode=excel&atdtype=draft";

		$url.="&d1=".$qdata;
		$url.="&d2=".$qdata2;

		if( isset($difmenor) && strlen($difmenor) > 0) 
			$url.="&difmenor=".$difmenor;

		if( isset($difmaior) && strlen($difmaior) >0 ) 
			$url.="&difmaior=".$difmaior;

			if( isset($falta) && strlen($falta) > 0) 
			$url.="&falta=".$falta;

		if( isset($excesso) && strlen($excesso) >0 ) 
			$url.="&excesso=".$excesso;
	...
	if( isset($visualizar) ) {
		...
		$sql.=" AND ( (ate.cliente IS NULL) OR (LENGTH(ate.cliente) < 1) )"; 

		if( (isset($difmenor) && strlen($difmenor) > 0) ||
			( isset($difmaior) && strlen($difmaior) > 0 )) {
			
			$sql.=" AND (";

			if( isset($difmenor) && strlen($difmenor) > 0 ) {
				$sql.=" (abs(diferenca) <= '$difmenor') ";
			...
			if( isset($difmaior) && strlen($difmaior) > 0 ) 
				$sql.= " (abs(diferenca) >= '$difmaior') ";

			$sql.= ") ";
		}

		if((isset($falta) && strlen($falta) > 0 ) || 
			(isset($excesso) && strlen($excesso) > 0 ) ) {
				$sql.=" AND (";	
			...
			<div align="center">
				...
				<form action="<?php echo $PHP_SELF ?>" method="post" enctype="multipart/form-data">
					<table width="50%" border="0" cellspacing="0" cellpadding="7">
						...
						<tr>
							<td align="right"><label for="difmenor">Diferença <=:</label></td>
							<td align="left"><input name="difmenor" type="text" id="difmenor" size="5" maxlength="5" placeholder="0.00"/></td>
						</tr>
						<tr>
							<td align="right"><label for="difmaior">Diferença >=:</label></td>
							<td align="left"><input name="difmaior" type="text" id="difmaior" size="5" maxlength="5" placeholder="0.00"/></td>
						</tr>
						<tr>
							<td align="right"><label for="diffalta">% falta:</label></td>
							<td align="left"><input name="diffalta" type="text" id="diffalta" size="5" maxlength="5" placeholder="0.00"/></td>
						</tr>
						<tr>
							<td align="right"><label for="difexcesso">% excesso:</label></td>
							<td align="left"><input name="difexcesso" type="text" id="difexcesso" size="5" maxlength="5" placeholder="0.00"/></td>
						</tr>
						<tr>
					...
					<TBODY>
						...
						if( $atendimento ) {
							...
							{	
								...
								$fileId = $atendimento->atendimento_id;
